...
import { Container, List, ListItem, ListItemText, Typography, Skeleton, Card, TextField, Button, Grid, IconButton, ListItemButton } from '@mui/material';
        ...
        <Container maxWidth="sm">
            ...
            <Grid container spacing={2} alignItems="center">
                ...
                <Grid item xs={12} sm={4} md={3}>
                    <Button onClick={handleNewList} color="primary" variant="contained" fullWidth>
                        Dodaj
                    </Button>
                </Grid>
            </Grid>
                    ...
                    {lists.map(({ id, name, date_created }) => (
                        <Card style={{ margin: '10px 0' }} key={id}>
                            <ListItemButton onClick={() => handleListClick(id)}>
                                <ListItemText
                                    primary={name}
                                    secondary={`Stworzona ${new Date(date_created).toLocaleDateString()}`}
                                />
